...
详情请参考：[附录：创建Kubernetes集群](./appendix_create_kubernetes.md)
...
回想之前我们之前初始化Kubernetes集群后，执行过的命令：

```bash
kubectl create deployment nginx-test --image=harbor.scs.buaa.edu.cn/library/nginx --replicas=2 --port=80
```
...
```bash
kubectl apply -f https://git-v1.scs.buaa.edu.cn/iobs/static_files/raw/main/kube/weave_scope/scope.yml
```
...
- 查看更详细的信息：`kubectl describe deployment nginx-test`
  ...
  Pod Template:
    ...
    Containers:
    nginx:
      Image:        harbor.scs.buaa.edu.cn/library/nginx
      Port:         80/TCP
      Host Port:    0/TCP
      Environment:  <none>
      Mounts:       <none>
...
其效果与我们在实验三中直接执行`kubectl create deployment nginx-test --image=harbor.scs.buaa.edu.cn/library/nginx --replicas=2 --port=80`效果是完全一样的。
...
Containers:
  nginx:
    ...
    Image:          harbor.scs.buaa.edu.cn/library/nginx
    Image ID:       harbor.scs.buaa.edu.cn/library/nginx@sha256:42bba58a1c5a6e2039af02302ba06ee66c446e9547cbfb0da33f4267638cdb53
    Port:           80/TCP
    Host Port:      0/TCP
    State:          Running
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: pi
          image: harbor.scs.buaa.edu.cn/library/perl
    ...
    spec:
      template:
        spec:
          containers:
            - name: hello
              image: harbor.scs.buaa.edu.cn/library/busybox
              args:
                - /bin/sh
                - -c
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: nginx
          image: harbor.scs.buaa.edu.cn/library/nginx:1.16
```
...
在刚刚的配置文件中将镜像修改为`harbor.scs.buaa.edu.cn/library/nginx:1.17`，然后在`spec`中添加滚动升级策略，改动后如下
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: nginx
          image: harbor.scs.buaa.edu.cn/library/nginx:1.17
```
...
Containers:
  nginx:
    ...
    Image:          harbor.scs.buaa.edu.cn/library/nginx:1.17
    Image ID:       harbor.scs.buaa.edu.cn/library/nginx@sha256:6f3b7f003243dca41dae46fe442e826df08e056a76cdd3e11e6f41cdaef
...
spec:
  containers:
    - name: busybox
      image: harbor.scs.buaa.edu.cn/library/busybox:1.24
      volumeMounts:
        - name: storage
...
- 部署nginx服务
  ...
  - 尝试使用滚动更新，对比前后主页版本 (可以自行创建Service通过代理访问网页，也可以直接curl ClusterIP访问。**注意浏览器缓存影响**)

- 部署持久化服务
  - 在集群中部署nfs环境
  - 创建pv和pvc
  - 创建mysql的Deployment和Service，确保绑定了pvc (**mysql的mount目录为/var/lib/mysql**)，创建MySQL时，可以使用镜像`harbor.scs.buaa.edu.cn/library/mysql:8`
